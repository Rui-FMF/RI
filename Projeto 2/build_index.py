...
class SPIMI:
    def __init__(self, tokenizer, signature, rank_method='lnc', memory_limit=800000, posting_limit=None):
        ...
        while(sys.getsizeof(index)<self.memory_limit):
            ...
            if(doc_id):
                ...
                for term in token_stream:
                    ...
                    if term in index:
                        index[term].append((doc_id,term_frequency))
                    else:
    ...
    def write_block(self, sorted_terms, index, block_file_name):
        ...
        with open(block_file_name, 'w', encoding='utf-8') as f:
            for term in sorted_terms:
                line = "%s %s\n" % (term, ''.join([";"+str(doc[0])+":"+str(doc[1]) for doc in index[term]]))
                f.write(line)
        return True
