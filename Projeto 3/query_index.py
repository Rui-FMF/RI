...
def boost_scores(scores, docs):

    for d in docs:
        docs_positions = docs[d]
        unique_terms = len(docs_positions)
        count = sum([len(t) for t in docs_positions])

        if unique_terms>1:
            sum_of_distances = 0
            for t1 in docs_positions:
                for t2 in docs_positions:
                    if t1[0]!=t2[0]:
                        sum_of_distances+= sum([int(i)-int(j) for i in t1 for j in t2 if int(i)>int(j)])

            boost = count/sum_of_distances
            scores[int(d)]+=boost
    return scores



def make_query(query, length, stopwords, stemming, ranking, query_ranking, boosting=True, k=1.2, b=0.75):
    ...
    for term in tokenized_query:
        ...
        with open('final_index.txt', "r", encoding='utf-8') as index:
            ...
            while True:
                ...
                if split_line[0]==term:
                    term_documents = [(i.split(':')[0],i.split(':')[1],list(i.split(':')[2].split(','))) for i in split_line[1:]]
                    break
        index.close()
        ...
        if boosting==True:
            for doc in term_documents:
                if doc[0] in docs_for_boosting:
                    docs_for_boosting[doc[0]].append(doc[2])
                else:
                    docs_for_boosting[doc[0]] = [doc[2]]
    ...
    if boosting==True:
        Scores = boost_scores(Scores, docs_for_boosting)
    ...
    while True:
        ...
        if choice=='1':
            if ranking=='bm25':
                ...
                top_docs, id_dict = make_query(query, int(length), stopwords, stemming, ranking, 'bm25', True, float(k), float(b))
            else:
                query = input("Enter a query ")
